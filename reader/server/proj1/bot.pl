...
jump_bot_cycle(Board, NewBoard, Piece, X, Y, Xf, Yf, [Xd , Yd | Resto]):- (
															jump(Board, NextBoard, Piece, X, Y, Xf, Yf, Xd, Yd),
															can_reJump(NextBoard, Piece, Xd, Yd),
															nl, display_board(NextBoard), nl,
															repeat , 
															(
																get_random_ortho(Xd, Yd, Xnew, Ynew),
																jump_bot_cycle(NextBoard, NewBoard, Piece, Xd, Yd, Xnew, Ynew, Resto)
															),
															nl, display_board(NewBoard), nl
														).
...
bot_movement_aux(Board, NewBoard, Piece, X, Y, Xf, Yf, Dests):- validate_destination(X, Y, Xf, Yf),
                                                            (
                                                               jump_bot_cycle(Board, NewBoard, Piece, X, Y, Xf, Yf, Dests);
                                                               (
																   replace_element(Board, CleanBoard, X, Y, v) ,check_ortho_adjacency(CleanBoard, Piece, Xf, Yf),
                                                                   check_restriction(Board, X, Y, Xf, Yf),
                                                                   (
...
random_cycle(Board, NewBoard, Piece, X, Y, Dests):- repeat,
												(
													get_random_dest(X, Y, Xf, Yf),
													bot_movement_aux(Board, NewBoard, Piece, X, Y, Xf, Yf, Dests)
												).						
...
choose_random_movement(Board, NewBoard, Piece, Plays, X, Y, Dests):- length(Plays, Num), N is Num - 1, N >= 1, random(0, N, Random),
														nth0(Random, Plays, Vect), nth0(0, Vect, X), nth0(1, Vect, Y),
														random_cycle(Board, NewBoard, Piece, X, Y, Dests).

choose_random_movement(Board, NewBoard, Piece, Plays, X, Y, Dests):- nth0(0, Plays, Vect), nth0(0, Vect, X), nth0(1, Vect, Y),
														random_cycle(Board, NewBoard, Piece, X, Y, Dests).


change_best_piece(Board, NewBoard, Piece, X, Y, Dests):- best_movement(Board, Piece, X, Y, _, N), get_smthing(X, Y, Xf, Yf, N),
											  bot_movement_aux(Board, NewBoard, Piece, X, Y, Xf, Yf, Dests).
											
choose_hard_movement(Board, NewBoard, Piece, Plays, X, Y, Dests):- length(Plays, Num), N is Num - 1, N >= 1, 
														repeat, (
															random(0, N, Random),
															nth0(Random, Plays, Vect), nth0(0, Vect, X), nth0(1, Vect, Y),
															change_best_piece(Board, NewBoard, Piece, X, Y, Dests)															
														).
														
choose_hard_movement(Board, NewBoard, Piece, Plays, X, Y, Dests):- nth0(0, Plays, Vect), nth0(0, Vect, X), nth0(1, Vect, Y),
													  change_best_piece(Board, NewBoard, Piece, X, Y, Dests).
														
bot_play(Board, NewBoard, Plays, Piece, X, Y, Dests):-  choose_random_movement(Board, NewBoard, Piece, Plays, X, Y, Dests).
hard_bot_play(Board, NewBoard, Plays, Piece, X, Y, Dests):-  choose_hard_movement(Board, NewBoard, Piece, Plays, X, Y, Dests).

lol(Board, NewBoard, Plays, Piece, [X, Y, Dests]):- bot_play(Board, NewBoard, Plays, Piece, X, Y, Dests).
